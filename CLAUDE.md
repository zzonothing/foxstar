...
- `ACCESS_KEY` — the shared union password; now used as the **gate for first-time PIN registration** on the login page (and for the emergency password-only entry)
- `ADMIN_KEY` — emergency admin password + the Bearer key for `api/admin-data.js`/`api/ingest.js`. Web admin rights normally come from `members.is_admin` instead (see below)
...
3. `api/data.js` checks the `fstar_session` cookie (HMAC-SHA256 over a timestamp, 1-day expiry, verified with `crypto.timingSafeEqual`) and whitelists the exact doc keys it will serve. It also implements **sliding session renewal**: on a valid request whose token is older than 12h it re-issues the cookie (`sessionCookie()` in `api/_lib/session.js`), so members who visit at least daily stay logged in and only >24h-idle sessions see the login page again. `SESSION_EPOCH` force-logout still applies to renewed tokens. On **authenticated** requests it serves the doc via `getDocEntry()` in `api/_lib/db.js` — a per-warm-instance cache validated by a single content-hash query per request (the body is only re-downloaded from the DB when the hash changed) — with `ETag` + `Cache-Control: private, no-cache` (see Caching below) and `Content-Type: application/javascript`. The unauth path returns **before any DB access**, so Neon cold starts never delay the login-redirect flow. On **unauthenticated** requests:
...
5. `/login.html` is a standalone minimal page (no header, no sticky/fixed layout, no view-transition, no speculation rules, no html2canvas). **Login is per-member: nickname + personal PIN** (`POST /api/auth` `mode:'member-login'`) — on success the server sets BOTH `fstar_session` (admin flag from `members.is_admin`) and `fstar_member` (identity). First-time users register via `mode:'member-claim'` (nickname + **shared `ACCESS_KEY` as the registration gate** + new PIN — stops strangers who only know the URL from claiming). On page load the page first tries `mode:'refresh'` — a valid 30-day `fstar_member` cookie silently re-issues the session, so members who visit at least monthly never retype anything. A small "비상: 공용 비밀번호로 입장" fallback keeps the old password-only entry (session without identity; `ADMIN_KEY` → admin session). The page then `location.replace()`s to the validated `return` path, whitelisted via `ALLOWED_PATHS` to `{/, /index.html, /notice.html, /raid.html, /submit.html, /solo.html, /shift.html, /stats.html, /history.html, /guide.html}` (path part only; a query string like `?s=S35` is allowed) to prevent open-redirect.
...
One page sits outside the nav: `login.html` (the auth gate).
...
**Updating data (no git, no redeploy):** the scraper repo POSTs to `/api/ingest` (Bearer `INGEST_KEY`, optionally gzip body with `X-Doc-Gzip: 1`) with `{ season, members?, characters? }` — the server upserts daily history, syncs the roster, and regenerates the `member.js` (merging only that season, preserving past seasons and `rank`) and `character.js` docs. Everything else (raid/solo/notice docs) is uploaded via `scripts/upload-doc.js` → `PUT /api/admin-data?key=<docKey>` (content validated against `DOC_RULES` in `api/_lib/db.js` before acceptance). When adding a new season: upload the new `member.js`/`raid.js` (append the season key), refreshed `character.js`, and later `solo.js` the same way.

**Member identity & writes:** members register their roster nickname once with a personal 4–8 digit PIN — normally at the **login page** (`/api/auth` `member-claim`, gated by the shared `ACCESS_KEY`; atomic `WHERE pin_hash IS NULL` guard), or in-page on `submit.html` (`/api/member-auth` `claim`, for users who entered via the emergency password). Success issues a 30-day `fstar_member` cookie (`m1.<uid>.<ts>.<sig>`, same HMAC secret, domain-separated prefix; sliding re-issue after 15 days) alongside the session. **Admin rights are member-based**: logging in as a member with `members.is_admin` (set by seed from `ADMIN_NAMES`) yields an admin session — used by notice CRUD, poll management, PIN reset. Write endpoints (`/api/submit`, `/api/poll` respond) require session + member cookie + `member_epoch` + `active` via `verifyMemberIdentity()` in `api/_lib/member.js`, and always take the uid from the cookie, never the request body. Admin recovery: `reset` (clear a member's PIN), `forceReauth` (bump `member_epoch`). Impersonation of an *unclaimed* nickname is blocked by the ACCESS_KEY gate at claim time and exposed by 409 on the real owner's claim — accepted for a 30-person community.
