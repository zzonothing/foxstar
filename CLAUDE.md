...
Serverless functions (CommonJS, all under `api/`): `auth.js` (login), `data.js` (gates `member/raid/character/solo/notice.js`), `sim.js` (gates simulator JSON), `notice.js` (admin notice CRUD → DB), `admin-data.js` (admin doc upload/list/delete), `ingest.js` (scraper intake: daily history + latest-doc regeneration), `member-auth.js` (nickname claim + personal PIN), `submit.js` (raid deck/score submissions), `poll.js` (schedule polls), `history.js` (daily-history time series). Shared modules in `api/_lib/`: `session.js` (session + member tokens), `compress.js` (ETag/gzip), `db.js` (Neon client, doc cache, validation), `member.js` (member-identity check), `history.js` (roster sync + daily upserts). Front-end pages share one script, `js/common.js` (helpers, header/nav injection, theme toggle, auth guard, modal + member-popup builder, URL-state helpers).
...
- **Upload a doc without redeploy**: `ADMIN_KEY=… node scripts/upload-doc.js <docKey>` (or `--all-sim`); >3MB files auto-gzip to dodge the 4.5MB request limit
...
- `ACCESS_KEY` — the shared password users enter at the login page
- `ADMIN_KEY` — the admin password (admin session tokens; also the Bearer key for `api/admin-data.js`)
...
Sensitive data (`member.js`, `raid.js`, `character.js`, `solo.js`, `notice.js`, and the simulator JSON under `sim/`) is **not** a static asset. It lives in the **`data_docs` table** (verbatim file text, keyed `member.js` / `sim/meta.json` / `sim/detail/<uid>.json` style), served through the gate functions. The copies under `api/_data/` are a **transition-period disk fallback only** (used when the DB errors or a doc row is missing) — they may lag the DB and will be removed together with `vercel.json`'s `includeFiles` once DB operation is verified (cutover). The flow:
...
2. `vercel.json` rewrites `/data/{member,raid,character,solo,notice}.js` → `/api/data?file=<name>`, and `/data/sim/*` → `/api/sim?file=<name>`.
3. `api/data.js` and `api/sim.js` both check the `fstar_session` cookie (HMAC-SHA256 over a timestamp, 1-day expiry, verified with `crypto.timingSafeEqual`) and whitelist the exact doc keys they will serve (`api/sim.js` additionally regex-validates the `detail/<uid>.json` path). `api/data.js` also implements **sliding session renewal**: on a valid request whose token is older than 12h it re-issues the cookie (`sessionCookie()` in `api/_lib/session.js`), so members who visit at least daily stay logged in and only >24h-idle sessions see the login page again. `SESSION_EPOCH` force-logout still applies to renewed tokens. On **authenticated** requests both serve the doc via `getDocEntry()` in `api/_lib/db.js` — a per-warm-instance cache validated by a single content-hash query per request (the body is only re-downloaded from the DB when the hash changed) — with `ETag` + `Cache-Control: private, no-cache` (see Caching below); `api/data.js` uses `Content-Type: application/javascript`, `api/sim.js` uses `application/json`. The unauth path returns **before any DB access**, so Neon cold starts never delay the login-redirect flow. They differ on **unauthenticated** requests:
...
- Never expose the gated data as static assets — it must always flow through `api/data.js`/`api/sim.js`. The transition-period copies under `api/_data/` are safe (underscore prefix keeps Vercel from serving them), but do not add new consumers of those files.
...
| `history.html` | Growth graphs + raid meta     | `config.js`, `member.js`, `raid.js` + `/api/history` |
...
- Simulator data (`sim/` doc keys): `meta.json` (rotations, kind labels, deck templates, element list), `union.json` (per-member precomputed damage vectors), and `detail/<uid>.json` (per-member breakdown). Consumed only by `sim.html`.
- `NOTICE` (from `notice.js`): flat array `[{ id, title, body, author, ts, editedTs? }]`, managed by `api/notice.js`.

**DB tables** (`scripts/schema.sql`; every table keyed by `union_id`, default 1): `data_docs` (the gated docs, verbatim text + `content_hash`), `members` (roster + personal-PIN scrypt hash; `pin_hash IS NULL` = unclaimed; leavers get `active=false`, never deleted), `app_settings` (`member_epoch` = force-reauth switch), `member_daily` / `character_daily` (one row per member/character per **KST day** — repeated ingest the same day updates the row, a new day inserts; the row holds that day's final state), `raid_submissions` (deck jsonb + damage + note, unique per season/uid/slot 1–3), `polls` + `poll_responses` (cascade delete).

**Updating data (no git, no redeploy):** the scraper repo POSTs to `/api/ingest` (Bearer `INGEST_KEY`, optionally gzip body with `X-Doc-Gzip: 1`) with `{ season, members?, characters? }` — the server upserts daily history, syncs the roster, and regenerates the `member.js` (merging only that season, preserving past seasons and `rank`) and `character.js` docs. Everything else (raid/solo/sim/notice docs) is uploaded via `scripts/upload-doc.js` → `PUT /api/admin-data?key=<docKey>` (content validated against `DOC_RULES` in `api/_lib/db.js` before acceptance). When adding a new season: upload the new `member.js`/`raid.js` (append the season key), refreshed `character.js`, later `solo.js`, and refreshed `sim/` docs the same way.

**Member identity & writes:** on top of the shared-password session, members claim their roster nickname once with a personal 4–8 digit PIN (`/api/member-auth` `claim`; atomic `WHERE pin_hash IS NULL` guard), then get a 30-day `fstar_member` cookie (`m1.<uid>.<ts>.<sig>`, same HMAC secret, domain-separated prefix; sliding re-issue after 15 days). Write endpoints (`/api/submit`, `/api/poll` respond) require session + member cookie + `member_epoch` + `active` via `verifyMemberIdentity()` in `api/_lib/member.js`, and always take the uid from the cookie, never the request body. Admin recovery: `reset` (clear a member's PIN), `forceReauth` (bump `member_epoch`). Impersonation of an *unclaimed* nickname is possible until the real owner claims (409 exposes it; admin resets) — accepted for a 30-person community.
...
Once DB serving is verified in production: remove the disk-fallback branches in `api/data.js`/`api/sim.js`, drop both `includeFiles` entries from `vercel.json`, delete `api/_data/**` from the repo (backup path: `scripts/dump-docs.js`; git history retains the files), and remove the now-unused `GH_TOKEN`/`GH_REPO`/`GH_BRANCH` env vars. Until then the fallback keeps the site working even with no DB seeded.
