...
| `history.html` | 성장 기록 — 유니온/멤버 보기 전환(기간 diff) + 성장 그래프 | `config.js`, `member.js`(auth-gate only) + `/api/history` |
