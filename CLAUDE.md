...
Sensitive data (`member.js`, `raid.js`, `character.js`, `solo.js`, `notice.js`, `char_details.js`) is **not** a static asset. It lives in the **`data_docs` table** (verbatim file text, keyed by filename), served through the gate function — and since the cutover the DB is the **only** source: there is no disk copy and no fallback. A DB outage returns `503`, a missing doc row returns `404` (both `no-store`), and the page shows its data-error screen. The flow:

1. HTML pages include them naively: `<script src="data/member.js"></script>`.
2. `vercel.json` rewrites `/data/{member,raid,character,solo,notice,char_details}.js` → `/api/data?file=<name>`.
...
- When adding a new top-level page: (1) add its path to the `ALLOWED_PATHS` whitelist in `login.html`, (2) add it to `NAV_ITEMS` in `js/common.js` if it should appear in the nav (4th element = feature key, and add that key to `FEATURE_KEYS` in `data/config.js`; call `featureGuard("<key>")` **before** the `__DATA_ERROR` check in the page's guard), (3) load `js/common.js` and call `authGuardOrRedirect()` in the page's main-script guard (copy the pattern from `index.html`/`raid.html`), and (4) copy the `카카오톡 인앱 우회 + 로딩 워치독` inline block and the non-render-blocking Google Fonts `<link>` into its `<head>`.
...
- `UNION` (from `member.js`): object keyed by season number as string, e.g. `UNION["40"]`. Each entry is `{ rank?, members: [...] }`. Entries per member always include `name`, `syncroLevel`, `uid`, `blaBlaLinkUId`, and optionally `role: "Leader" | "Officer"`. The **latest season only** also includes per-member gameplay fields: `normal`, `hard`, `tribeTower`, `overclock`, `outpost: { common, attack, defense, support, missilis, elysion, tetra, pilgrim, abnormal }`. To find the latest season use `Math.max(...Object.keys(UNION).map(Number))` (pages use `Object.keys(UNION).map(Number).sort((a,b)=>a-b)` and take the last element).
  - **`normal`/`hard` are campaign *labels* (`"46-40"`), not numbers, and the conversion happens in the scraper, not here.** blablalink returns a stage **id** (`<난이도 1><챕터 3><챕터 내 순번 3>`, 6=노말 7=하드); einkk's `decode_campaign` maps it through the game CDN's stage table and falls back to the raw id when the table doesn't know it — which is what a newly released chapter looks like. Never decode the id arithmetically: the trailing 3 digits count branch stages (`46-14A-1` …) too, so the offset differs per chapter (46장 순번 44 = `46-40`, 48장 순번 44 = `48-36`). The portal's job is only to not show a bare 7-digit id — `stageLabel`/`stageTitle`/`stageSortValue` in `js/common.js` render it as `"48장"` with a tooltip and keep sorting on the same axis. The real fix is on the scraper side (einkk auto-refreshes the table when it meets an unknown id). `CAMPAIGN_MAX_STAGE` in `common.js` is the red "clear" highlight threshold — one line to bump when a new final stage ships.
...
- `CHAR_DETAILS` (from `char_details.js`): keyed by `name_code` as a string (e.g. `CHAR_DETAILS["5156"]`). Static per-character game data — name/element/class, `shot` (weapon stats), `skill1`/`skill2`/`ulti` (skill meta with parsed `effects`). Union-independent (same content on every deployment). Built and uploaded by the scraper repo (einkk `tools/build_char_details.py` → `tools/upload_char_details.py`, ~1.5MB so it gets the `JSON.parse` wrap). No page consumes it yet — the doc key is registered so the upload/serving path works ahead of UI.
...
**Scrape timing decides the day's value.** `member_daily`/`character_daily` are keyed by KST day, and `api/ingest.js` stamps the row with `kstToday()` **evaluated when the POST arrives**. Re-ingesting the same day overwrites, so **the last ingest that lands inside a KST date is that date's stored state** — anything after it rolls into the next day's row. A scraper on 0/6/12/18 therefore stores each day's *18:00* state, not its end state. Schedule the last run late in the day (the scraper repo's README recommends 4/10/16/22) — this also makes the weekly survivor (Sunday, see 히스토리 솎아내기) a true end-of-week snapshot. **Do not schedule near midnight**: unions are ingested sequentially (the second POST arrives minutes later), so a run starting 23:5x can land union A on day D and union B on D+1, leaving B with no row for D and an early-overwritten row for D+1.

**Updating data (no git, no redeploy):** the scraper repo POSTs to `/api/ingest` (Bearer `INGEST_KEY`, optionally gzip body with `X-Doc-Gzip: 1`) with `{ season, members?, characters? }` — the server upserts daily history, syncs the roster, regenerates the `member.js` (merging only that season, preserving past seasons and `rank`) and `character.js` docs, and finally prunes old history (below). Everything else (raid/solo/notice/char_details docs) is uploaded via `scripts/upload-doc.js` → `PUT /api/admin-data?key=<docKey>` (content validated against `DOC_RULES` in `api/_lib/db.js` before acceptance); `char_details.js` normally comes straight from the scraper repo's `tools/upload_char_details.py`, which PUTs to the same endpoint. When adding a new season: upload the new `member.js`/`raid.js` (append the season key), refreshed `character.js`, and later `solo.js` the same way.

**Reading a doc back (read-modify-write tools):** `GET /api/admin-data?key=<docKey>` (same Bearer `ADMIN_KEY`) returns the **verbatim stored doc** — no `jsonParseWrap`, so what you get is exactly what you can PUT back — with `ETag` = `content_hash` and gzip when the client sends `Accept-Encoding: gzip` (required for `character.js`: 5.9MB raw exceeds the 4.5MB response limit). ★ Do **not** point such a tool at `/data/<file>` — that is the member gate, and a cookie-less request gets `200 OK` with body `window.__AUTH_REQUIRED=true;` (deliberately not a 4xx, so the `<script src>` still runs), so the tool sees no HTTP error and a body with no `const … =` envelope. That exact mistake is what the scraper repo's `tools/set_rank.py` hit. Pass the ETag back as `If-Match: <hash>` on the PUT to get an optimistic-concurrency guard (`412` if the doc changed meanwhile) — without it a slow read-modify-write silently reverts whatever `/api/ingest` merged in between. Omitting `If-Match` keeps the old unguarded overwrite (what `scripts/upload-doc.js` does, since it replaces the whole doc).
...
**유니온별 기능 on/off**: `SITES` 항목의 `features` 로 nav 메뉴를 끈다 — `features: { notice: false, submit: false, solo: false }` (나증단 현재 설정). 키는 `config.js` 상단의 `FEATURE_KEYS`(`notice`·`raid`·`submit`·`solo`·`shift`·`stats`·`history`·`guide`)이고, **생략하거나 `false` 를 지우면 다시 켜진다.** 홈은 착지 페이지라 끌 수 없다. `CONFIG.features` 는 모든 키가 채워진 boolean 맵으로 정규화돼 나오므로 소비자는 기본값 처리를 하지 않아도 된다.

동작은 두 군데다: (1) `js/common.js` 의 `renderHeader` 가 `NAV_ITEMS`(4번째 원소가 기능키)를 `featureOn()` 으로 걸러 nav 에서 뺀다, (2) 각 페이지가 본 스크립트 앞에서 `featureGuard("<키>")` 를 호출해, 꺼진 유니온에서 주소로 직접 열면 안내 화면으로 바꾼다. **`featureGuard` 호출은 `__DATA_ERROR` 검사보다 앞에 둔다** — 끄는 유니온에는 그 문서 행이 없어 404 가 나므로, 순서가 뒤바뀌면 '데이터 오류'가 먼저 뜬다. 새 페이지를 추가할 때 이 순서를 지킬 것.

★ 이것은 **보안 경계가 아니라 UI 토글**이다. `config.js` 는 게이트를 타지 않는 공개 파일이라 누구나 값을 볼 수 있고, 끈 페이지의 데이터도 여전히 `api/data.js` 인증이 지킨다. 특정 유니온에서 데이터 자체를 감춰야 한다면 그 문서를 DB 에 넣지 않는 것이 실제 방어선이다.
...
3. `raid.js`/`solo.js`/`notice.js` 스텁 업로드 (`const RAID = {};` / `const SOLO = [];` / `const NOTICE = [];`) — 문서 행이 없으면 `api/data.js` 가 404 를 반환해 페이지가 데이터 오류를 띄운다. 파일을 만들어 `node scripts/upload-doc.js raid.js ./raid.js` 처럼 경로를 넘긴다 (`char_details.js` 는 소비 페이지가 없어 스텁이 필요 없다 — einkk 의 `tools/upload_char_details.py` 로 실데이터를 올리면 된다)
